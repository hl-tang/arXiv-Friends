...
from .schemas import RegisterIn, LoginIn, NotesPostIn, NotesGetOut
...
def user_like_paper(request, paper_id: str):
    ...
    if existing_record:
        ...
        UserLikePaper.objects.create(
            user_id_id=request.user.id, paper_id_id=paper_id)
...
def post_notes(request, payload: NotesPostIn):
    existing_record = UserBrowsePaperHistory.objects.filter(
        user_id_id=request.user.id, paper_id_id=payload.paper_id).first()
    if existing_record:
        existing_record.notes = payload.notes
        existing_record.save()
    # 其实不需要else了，因为先访问simplify进入detailpaper页面，那肯定在浏览历史里加入记录了
    else:
        UserBrowsePaperHistory.objects.create(user_id_id=request.user.id,
                                              paper_id_id=payload.paper_id,
                                              notes=payload.notes)
...
def get_notes(request, paper_id: str):
    existing_record = UserBrowsePaperHistory.objects.filter(
        user_id_id=request.user.id, paper_id_id=paper_id).first()
    if existing_record:
